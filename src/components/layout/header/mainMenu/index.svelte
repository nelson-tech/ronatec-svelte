<script lang="ts">
	...
	type GetDesktopLinkStyleType = ({ open, path }: DesktopLinkStyleProps) => string

	const getDesktopLinkStyle: GetDesktopLinkStyleType = ({ open, path }): string => {
		const current = $page.url.pathname.slice(0, path.length) === path && path !== "/"
...
<div class="hidden h-full lg:flex items-center">
	<PopoverGroup
		><div class="h-full flex items-center space-x-2 text-sm font-medium text-gray-600">
